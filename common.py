...
class PacketUtils:
    ...
    def evade(self, target, msg, ttl):
        ...
        for i in range(len(msg)-1):
            self.send_pkt(payload=msg[i:i+1], flags="A", seq=ack+i, ack=seq_1, sport=source_port, dip=target)
            self.send_pkt(payload='x', ttl=ttl, flags="A", seq=ack+i, ack=seq+1, sport=source_port, dip=target)

        timeout = time.time() + 5
        while (self.packetQueue.qsize() > 0):
            response = self.get_pkt(max(0, timeout - time.time()))
            if response == None:
                return "ERROR"
